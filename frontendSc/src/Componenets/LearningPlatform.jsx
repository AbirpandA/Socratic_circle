    ...
    const fetchPosts = async () => {
      try {
        ...
        const formattedPosts = data.map(post => ({
          ...
          category: post.category || "General", // Ensure category is used properly
          timeAgo: "Just now",
        ...
        <div className="max-w-3xl mx-auto">
          ...
          <div className="flex space-x-4 mb-8 overflow-x-auto pb-2">
            {["All", "General", "Art", "Literature", "Science", "Music"].map(category => (
              <button
                key={category}
                className={`px-4 py-2 rounded-full border text-stone-600 font-serif transition-all 
                  ${selectedCategory === category 
                    ? "bg-stone-800 text-white border-stone-800" 
                    : "border-stone-200 hover:bg-stone-800 hover:text-white hover:border-stone-800"}`}
            ...
            {filteredPosts.length > 0 ? (
              filteredPosts.map(post => (
                <div key={post.id} className="bg-white p-8 rounded-lg border border-stone-200 hover:border-stone-300 transition-all shadow-sm hover:shadow-md">
                  ...
                  {post.image && <img src={post.image} alt="Post Visual" className="w-full h-auto rounded-lg mb-4" />}
                  {post.videoUrl && (
              ...
              <p className="text-center text-stone-600">No posts available in this category.</p>
            )}
          </div>
